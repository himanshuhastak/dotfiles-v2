...
def _client(args):
    ...
    if not url:
        raise SystemExit(
            'GitLab URL required — run: dotfiles secrets set gitlab_url'
        )
    if not token:
        raise SystemExit(
...
def print_test_report(report):
    ...
    print('  verify_ssl:  {}'.format(report.get('verify_ssl')))
    print('  user:     {} ({})'.format(
...
def build_parser():
    ...
    test_p.add_argument('--insecure', action='store_true',
                        help='disable TLS certificate verification (internal GitLab)')
