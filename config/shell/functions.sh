# config/shell/functions.sh — portable functions (bash + zsh).
...
cd() {
  ...
  if [ $# -eq 1 ] && command -v z >/dev/null 2>&1; then
    z "$1"
    return
  fi
  builtin cd "$@"
